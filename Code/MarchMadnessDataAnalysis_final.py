...
# Convert y_test from one-hot encoding (if that's the case) to class labels
# If y_test is already a 1D array of class labels, this step is not needed
# y_test_labels = np.argmax(y_test, axis=1)  # assuming y_test is one-hot encoded

# If y_test is a 1D array of class labels, you can skip this step
y_test_labels = y_test  # Already in correct format

# Binarize the labels for AUC calculation (this is necessary for multi-class classification)
y_test_binarized = label_binarize(y_test_labels, classes=[0, 1, 2, 3])  # adjust the classes if needed
...
def simulate_tourney(round_df, mode, best_model, predictors):
    removed_cols = ['YEAR', 'BY YEAR NO', 'BY ROUND NO', 'TEAM NO', 'TEAM', 'SEED', 
                    'ROUND', 'CURRENT ROUND', 'CONF', 'CONF ID', 'QUAD NO', 
                    'QUAD ID', 'TEAM ID', 'BID TYPE', 'OUTCOME', 'TYPE']
    selected_cols = [col for col in round_df.columns if col not in removed_cols]
    game_count = 32  # Initial game count for round 1
    curr_df = round_df.copy()

    # Ensure predictors match the model's training features
    missing_cols = [col for col in predictors if col not in round_df.columns]
    if missing_cols:
        print(f"Warning: Missing columns from the dataset: {missing_cols}")
        predictors = [col for col in predictors if col in round_df.columns]  # Remove missing columns
    ...
    for i in range(6):  # Loop through each round
        ...
        for i in range(int(game_count)):  # Ensure game_count is an integer for range
            ...
            game_pred = best_model.predict_proba(game_matrix)[:, 1]  # Get probabilities
...
# Call the function
predictors = list(X_train.columns)  # List of training features
