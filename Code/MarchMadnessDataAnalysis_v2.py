...
train = cd.loc[cd['YEAR'] <= 2022]
test = cd.loc[cd['YEAR'] == 2023]

# Set target variables for training and test sets
y_train = train['OUTCOME'].astype(int)
y_test = test['OUTCOME'].astype(int)

# Filter out non-numeric columns from the correlation DataFrame
numeric_cols = train.select_dtypes(include=[float, int]).columns
corr_numeric = train[numeric_cols].copy()

# Keep only rows from the initial dataframe where 'CURRENT ROUND' is 64
corr_numeric = corr_numeric[corr_numeric['CURRENT ROUND'] == 64]

# Calculate correlations with the 'ROUND' column and select features above threshold
cor = corr_numeric.corr()
threshold = 0.45  # Define threshold for feature selection
selected_predictors = cor['ROUND'].abs()[cor['ROUND'].abs() > threshold].index.drop(['ROUND'], errors='ignore')
print("Selected Predictors:\n", selected_predictors)

# Filter the training and test sets based on the selected predictors
X_train = train[selected_predictors]
...
# Confirm the final shapes of scaled train and test sets
...
def objective(space):
    ...
    # Evaluation sets with corrected target variable names
    evaluation = [(X_train_scaled, y_train), (X_test_scaled, y_test)]
    clf.fit(X_train_scaled, y_train, eval_set=evaluation, verbose=False)

    # Calculate predictions and define the loss function
    pred = clf.predict(X_test_scaled)
    loss = 1 - clf.score(X_test_scaled, y_test)  # You can adjust this to your specific loss function
...
# Fit the model using `y_train` as the target variable
...
# Ensure test has the exact predictor columns in order
test = test[train[predictors].columns]

# Convert to NumPy array to strip any pandas metadata that could interfere
...
# Fit the model on the full training data with evaluation on the validation set
...
def simulate_tourney(round_df, mode, best_model, predictors):
    removed_cols = ['YEAR', 'BY YEAR NO', 'BY ROUND NO', 'TEAM NO', 'TEAM', 'SEED', 'ROUND', 'CURRENT ROUND', 'CONF', 'CONF ID', 'QUAD NO', 'QUAD ID', 'TEAM ID', 'BID TYPE', 'OUTCOME', 'TYPE']
    selected_cols = round_df.columns[~round_df.columns.isin(removed_cols)]  # Remove non-essential columns
    game_count = 32  # Initial game count for round 1
    curr_df = round_df.copy()

    # Ensure predictors match the model's training features
    model_features = best_model.get_booster().feature_names  # Get the feature names the model was trained with
    missing_cols = [col for col in predictors if col not in model_features]
    if missing_cols:
        print(f"Warning: Missing columns from model's training set: {missing_cols}")
        predictors = [col for col in predictors if col in model_features]  # Remove missing columns

    for i in range(6):  # Loop through each round
        team = 0
        next_round = pd.DataFrame()

        # Adjust the dataframe based on the round (cut off columns that are irrelevant for this round)
        if i != 0:
            round_df = round_df.iloc[:, :round_df.columns.get_loc('OPP YEAR')]

        round_df = round_df.fillna(0)  # Replace all N/A values with 0
        round_df['TYPE'] = np.where(round_df.index % 2 == 0, 0, 1)  # Assign TYPE value based on row index

        # Calculate the difference for each stat to represent the matchup odds
        odds = round_df[selected_cols].diff()
        odds_df = odds.iloc[1::2]
        evens = -round_df[selected_cols].diff()
        evens = evens[selected_cols].shift(-1)
        ...
        # Combine the relevant data to form the current matchup round
        round_df = pd.concat([temp, evens_df], axis=1)
        round_df = pd.concat([round_df, odds_df], axis=0)
        round_df = round_df.groupby(level=0).sum()
        round_df = round_df.sort_index(ascending=True)

        # Select the winning and losing teams for the matchups
        win1 = round_df.loc[round_df['TYPE'] == 1].reset_index(drop=True)
        loss1 = round_df.loc[round_df['TYPE'] == 0].add_prefix('OPP ').reset_index(drop=True)
        
        win2 = round_df.loc[round_df['TYPE'] == 1].add_prefix('OPP ').reset_index(drop=True)
        loss2 = round_df.loc[round_df['TYPE'] == 0].reset_index(drop=True)

        # Combine dataframes for all matchups in the current round
        temp1 = pd.concat([win1, loss1], axis=1)
        temp2 = pd.concat([loss2, win2], axis=1)

        round_df = pd.concat([temp1, temp2], axis=0).sort_index().reset_index(drop=True)

        # Process each game in the round
        for i in range(int(game_count)):  # Ensure game_count is an integer for range
            game = round_df.iloc[team:team+2]  # Get the current matchup

            # Fill missing values in the predictors with 0
            game[predictors] = game[predictors].fillna(0)

            # Prepare the game data for prediction
            game_matrix = game[predictors].values
            game_pred = best_model.predict(game_matrix)  # Make predictions

            # Select the winner based on the prediction
            winner = game.iloc[0] if game_pred[0] > game_pred[1] else game.iloc[1]

            # If mode is 1, use an alternative winner selection method
            if mode == 1:
                winner = get_leverage(winner, team, game, game_pred)

            winner = winner.to_frame().T  # Convert the winner to a DataFrame
            winner = curr_df[curr_df['TEAM'].isin(winner['TEAM'])]
            next_round = pd.concat([next_round, winner], axis=0)
            team += 2  # Move to the next matchup

        next_round = next_round.reset_index(drop=True)

        # Set the new odds and evens for the next round
        odds = next_round.iloc[1::2]
        evens = next_round.iloc[::2]

        # Adjust the odds and evens for game count if it's not the last round
        if game_count != 1:
            odds = odds.set_index(np.arange(0, game_count, 2))
            evens = evens.set_index(np.arange(1, game_count, 2))

        # Combine the results for the current round
        opp = pd.concat([evens, odds], axis=0)
        opp = opp.add_prefix('OPP ').sort_index(ascending=True)
        next_round = pd.concat([next_round, opp], axis=1)
        next_round = next_round.reset_index(drop=True)
        next_round['CURRENT ROUND'] = game_count
        round_df = next_round.copy()

        # Reduce the game count by half for the next round
        game_count /= 2

        # Display the current round outcomes
        display(round_df[['YEAR', 'TEAM', 'SEED', 'CURRENT ROUND']])

# Update the 'predictors' list to match the model's training set
predictors = best_model.get_booster().feature_names

#simulate_tourney(curr_tourney, mode=0, best_model=best_model, predictors=predictors)
...
# Replace 'round_df' with the actual dataset you are passing to the function (e.g., curr_tourney)
missing_columns = [col for col in best_model.get_booster().feature_names if col not in curr_tourney.columns]
if missing_columns:
...
#simulate_tourney(curr_tourney, 0,best_model=best_model, predictors=predictors)
...
#simulate_tourney(curr_tourney, 1,best_model=best_model, predictors=predictors)
